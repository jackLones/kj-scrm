				...
				janus = new Janus(
					{
							...
							janus.attach(
								{
									...
									onlocalstream : function (stream) {
										...
										if (m7$('#myvideo').length === 0)
											m7$('body').append('<video class="rounded centered" id="myvideo" width=320 height=5 autoplay muted="muted"/>');
										Janus.attachMediaStream(m7$('#myvideo').get(0), stream);
										m7$("#myvideo").get(0).muted = "muted";
										// No remote video yet
										m7$('body').append('<video class="rounded centered" id="waitingvideo" width=320 height=5 />');
										var videoTracks = stream.getVideoTracks();
									},
									onremotestream: function (stream) {
										Janus.debug(" ::: Got a remote stream :::");
										Janus.debug(stream);
										m7$('body').append('<video class="rounded centered hide" id="remotevideo" width=320 height=5 autoplay />');
						...
						error             : function (error) {
							...
							if (error === 'Lost connection to the gateway (is it down?)' || (error && error.indexOf('Probably a network error, is the gateway down?') !== -1) && window.userLogin) {
								started = false
