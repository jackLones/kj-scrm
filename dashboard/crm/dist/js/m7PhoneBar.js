...
sevenDeclare("moorCall.Phone", null, {
	...
	dialout       : function (phoneNum, actionId, type) {
		if (type == 'smallPhone') {
			//小号
			...
			var account = moorCall.config.account_id;//需要调整成自己账号的账户编号
			var secret = moorCall.config.api_secret;//需要调整成自己账号的账户秘钥
			...
			m7$.ajax({
				url        : moorCall.config.api_host + "/v20160818/rlxh/dialout/" + account + "?sig=" + sig,
				type       : 'post',
