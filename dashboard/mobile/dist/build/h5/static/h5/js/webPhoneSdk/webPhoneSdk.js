							...
							janus.attach(
								{
									...
									onlocalstream : function (stream) {
										...
										if (m7$('#myvideo').length === 0)
											m7$('body').append('<video class="rounded centered" id="myvideo" width=5 height=5 autoplay muted="muted"/>');
										Janus.attachMediaStream(m7$('#myvideo').get(0), stream);
										m7$("#myvideo").get(0).muted = "muted";
										// No remote video yet
										m7$('body').append('<video class="rounded centered" id="waitingvideo" width=5 height=5 />');
										var videoTracks = stream.getVideoTracks();
									},
									onremotestream: function (stream) {
										Janus.debug(" ::: Got a remote stream :::");
										Janus.debug(stream);
										m7$('body').append('<video class="rounded centered hide" id="remotevideo" width=5 height=5 autoplay />');
