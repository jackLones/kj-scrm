<?php
	...
	/**
	 * This is the model class for table "{{%work_contact_way_redpacket_date}}".
	 *
	 * @property int                     $id
	 * @property int                     $way_id      渠道活码红包活动表ID
	 ...
	 */
	class WorkContactWayRedpacketDate extends \yii\db\ActiveRecord
	{
		...
		{
			return '{{%work_contact_way_redpacket_date}}';
		}
		...
		{
			if (!empty($data)) {
				$dateData = WorkContactWayRedpacketDate::find()->andWhere(['way_id' => $way_id, 'type' => 1])->all();
				foreach ($dateData as $v) {
					WorkContactWayRedpacketDateUser::deleteAll(['date_id' => $v->id]);
				}

				static::deleteAll(['way_id' => $way_id, 'type' => 1]);
				foreach ($data as $val) {
					/*if ($flag == 1 || empty($val['id'])) {
						$contactWayDate              = new WorkContactWayRedpacketDate();
					...
					$dataUser = $val['time'];
					WorkContactWayRedpacketDateUser::setData($dataUser, $contactWayDate->id);
				}
			}
		...
		public static function add ($way_id, $weekUser, $day)
		{
			...
			if (empty($user)) {
				$user              = new WorkContactWayRedpacketDate();
			...
			WorkContactWayRedpacketDateUser::setData($dataUser, $user->id);
		...
		public static function getChooseDate ($wayId, $corpId)
		{
			...
			if (!empty($wayDate)) {
				foreach ($wayDate as $key => $date) {
					...
					$time                      = [];
					$dateUser                  = WorkContactWayRedpacketDateUser::find()->where(['date_id' => $date['id']])->asArray()->all();
					if (!empty($dateUser)) {
		...
		public static function getData ($wayId, $day, $corpId)
		{
			...
			$time = [];
			if (!empty($mon)) {
				$dateUser = WorkContactWayRedpacketDateUser::find()->where(['date_id' => $mon->id])->asArray()->all();
				if (!empty($dateUser)) {
