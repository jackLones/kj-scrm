<?php
	...
	/**
	 * This is the model class for table "{{%work_contact_way_group}}".
	 *
	 * @property int                   $id
	 ...
	 * @property User                  $u
	 * @property WorkContactWayGroup[] $groups
	 */
	class WorkContactWayGroup extends \yii\db\ActiveRecord
	{
		/**
		 * {@inheritdoc}
		 */
		...
		public static function setNoGroup ($uid, $corp_id, $is_update = 0)
		{
			...
			if (empty($group)) {
				...
				if ($group->validate() && $group->save()) {
					if (!empty($is_update)) {
						WorkContactWay::updateAll(['way_group_id' => $group->id], ['corp_id' => $corp_id, 'is_del' => 0, 'way_group_id' => NULL]);
					}
				}
			}
			...
			if (!empty($topGroupData)) {
				foreach ($topGroupData as $groupData) {
					...
					if ($withSubGroup) {
						...
						//是否查询分组下的个数
						if ($withNum) {
							$workContactWay   = WorkContactWay::find()->where(['corp_id' => $corp_id, 'way_group_id' => $groupData->id, 'is_del' => 0]);
			...
			if (!empty($subGroupList)) {
				foreach ($subGroupList as $subGroup) {
					...
					if ($withSubGroup) {
						...
						if ($withNum) {
							$workContactWay      = WorkContactWay::find()->where(['corp_id' => $corp_id, 'way_group_id' => $subGroup->id, 'is_del' => 0]);
							$num                 = $workContactWay->count();
		...
		//根据父级id获取子id
		public static function getSubGroupId ($group_id,$corp_id)
		{
			$groupIdList = [$group_id];
			$GroupALL = static::find()->where(['corp_id' => $corp_id, 'status' => 1])->select('id,parent_id')->orderBy("parent_id desc")->asArray()->all();
			foreach ( $GroupALL as $value){
				if(in_array($value["parent_id"],$groupIdList)){
