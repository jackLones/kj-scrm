<?php
	...
	/**
	 * This is the model class for table "{{%work_contact_way_date}}".
	 *
	 * @property int            $id
	 ...
	 * @property int            $day         1到7代表周一到周日
	 * @property string         $create_time 创建时间
	 *
	 * @property WorkContactWay $way
	 */
	class WorkContactWayDate extends \yii\db\ActiveRecord
	{
		...
		public static function setData ($data, $way_id, $flag = 0)
		{
			if (!empty($data)) {
				if ($flag == 0){
					...
					if (!empty($dateUser)) {
						...
						foreach ($dateUser as $user) {
							WorkContactWayDateUser::deleteAll(['date_id' => $user->id]);
						}
					}
				...
				foreach ($data as $val) {
					$contactWayDate              = new WorkContactWayDate();
					...
					$dataUser = $val['time'];
					WorkContactWayDateUser::setData($dataUser, $contactWayDate->id);
		...
		public static function add ($way_id, $weekUser, $day)
		{
			...
			WorkContactWayDateUser::setData($dataUser, $user->id);
		...
		public static function getChooseDate ($wayId, $corpId)
		{
			...
			if (!empty($wayDate)) {
				foreach ($wayDate as $key => $date) {
					...
					$time                      = [];
					$dateUser                  = WorkContactWayDateUser::find()->where(['date_id' => $date['id']])->asArray()->all();
					if (!empty($dateUser)) {
		...
		public static function getData ($wayId, $day, $corpId)
		{
			...
			$time = [];
			if (!empty($mon)) {
				$dateUser = WorkContactWayDateUser::find()->where(['date_id' => $mon->id])->asArray()->all();
				if (!empty($dateUser)) {
		...
		public static function getEditDateUser ($id, $type, $day)
		{
			$userId  = [];
			$partyId = [];
			$date    = WorkContactWayDate::findOne(['way_id' => $id, 'day' => $day]);
			if (empty($date)) {
				throw new InvalidDataException("参数不正确");
			}
			$dateUser = WorkContactWayDateUser::find()->where(['date_id' => $date->id])->asArray()->all();
