			...
			<div id="actionListBox" class="ibox float-e-margins ">
				<div class="ibox-content">
					<div class="form-group">
						<div class="row">
							<form id="searchForm" method="get" action="/admin/short-message/user-template">
								<div class="col-lg-2">
								...
								<div class="col-lg-2">
									<select class="form-control select" name="sid" id="sid">
										...
										<?php foreach($signArr as $k=>$v):?>
											<option <?= ($sid == $k)?'selected':'';?> value="<?=$k;?>"><?=$v;?></option>
										<?php endforeach;?>
									</select>
								</div>
								<div class="col-lg-2">
									<select class="form-control select" name="tid" id="tid">
										...
										<?php foreach($typeArr as $k=>$v):?>
											<option <?= ($tid == $k)?'selected':'';?> value="<?=$k;?>"><?=$v;?></option>
										<?php endforeach;?>
									</select>
								</div>
								<div class="col-lg-2">
					...
					<table class="ui-table ui-table-list default no-paging footable-loaded footable" style="font-size: 13px;">
						...
						<?php foreach($templateArr as $template):?>
							<tr class="widget-list-item action-info-<?=$template['id'];?>">
								<td class="action-id"><?=$template['id'];?></td>
								<td><?=$template['apply_time'];?></td>
								<td>
									<?= !empty($template['account']) ? \app\util\SUtils::hideString($template['account']) : '系统默认';?>
								</td>
								<td><?=!empty($signArr[$template['sign_id']]) ? $signArr[$template['sign_id']] : '--';?></td>
								<td><?=!empty($typeArr[$template['type_id']]) ? $typeArr[$template['type_id']] : '--';?></td>
								...
								<td>
									<?php
										...
										if($template['status'] == 1){
											...
											$msg = '未通过，<span style="color:red">'.$template['error_msg'].'</span>';
				...
				<div class="row data-box hide">
					...
					<form role="form">
						...
						<div class="form-group error_msg" style="display: none;">
							...
							<input type="text" class="form-control menu-input" id="error_msg" value="" maxlength="250" placeholder="不通过原因">
						</div>
					</form>
				</div>
			</div>
...
<script>
	...
	function clear(){
		window.location.href = '/admin/short-message/user-template';
	}
	$(function(){
		$("#uid,#sid,#tid").chosen({
	...
	$("#actionModal .btn-confirm").click(function () {
		...
		$.ajax({
			url     : "/admin/short-message/template-status",
			type    : "POST",
			data    : postData,
			dataType: "JSON",
