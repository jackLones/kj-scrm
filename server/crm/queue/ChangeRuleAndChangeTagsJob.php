<?php
	...
	/**
	 * 根据条件打标签-编辑和删除标签规则
	 * File: queue/ChangeRuleAndChangeTagsJob.php
	 ...
	 *
	 */
	class ChangeRuleAndChangeTagsJob extends BaseObject implements JobInterface
	{
		...
		public function execute ($queue)
		{
			...
			set_time_limit(0);
			try {
				\Yii::error('param_id:' . $this->param_id, 'ChangeRuleAndChangeTagsJob');

				//查询所有条件标签
				$workTaskTag = WorkTaskTag::findOne(['corp_id' => $this->corp_id, 'id' => $this->param_id]);
				if ($workTaskTag) {
					if (in_array($this->type, [0, 1])) {
						$date = $workTaskTag->getUserTagByRule($this->uid, $this->type);
						\Yii::error($date, 'ChangeRuleAndChangeTagsJob');
